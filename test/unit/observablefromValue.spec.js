...
describe('vm.$observableFromValue', () => {
  it('should emit the current value on subscribe', () => {
    ...
    expect(observer1.nextSpy).toHaveBeenCalledTimes(1)
    vm.x = 'changed_value'
    vm.$observableFromValue('x').subscribe(observer2)
    expect(observer2.value).toBe('changed_value')
    expect(observer2.nextSpy).toHaveBeenCalledTimes(1)
  })
  ...
  it('should emit a new value on change', done => {
    ...
    expect(vm.x).toBe('intial_value')
    vm.x = 'changed_value'
    expect(observer.nextSpy).toHaveBeenCalledTimes(1)
    vm.$nextTick(() => {
      expect(observer.value).toBe('changed_value')
      expect(observer.nextSpy).toHaveBeenCalledTimes(2)
      done()
    })
  })
  ...
  it('should complete on $destroy', () => {
    ...
    vm.$mount()
    expect(value$.completeSpy).not.toHaveBeenCalled()
    vm.$destroy()
    expect(value$.completeSpy).toHaveBeenCalled()
  })
